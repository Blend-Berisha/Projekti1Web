<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="Lojtaret.css">
</head>
<body>
    <header>
        <nav class="navigation">
            <img src="data:image/jpeg;base64,"
                alt="">
                <h3><a href="#">Bad News Eagles</a></h3>
            <ul>
                <li><a href="index.html">Home</a></li>
                <li><a href="#">Lojtaret</a></li>
                <li><a href="shop.html">Shop</a></li>
                <li><a href="LogIn.html">Log-in</a></li>
            </ul>
        </nav>
    </header>
    <main>
        <div id="kontenti">
        
            <h2>Bad News Eagle Player's </h2>
            <img id="slideshow">
        </div>

    
        <script>
            let i = 0;
            let imgArray = ['juanflatroo.png','Sinnopsy.webp','rigoN.png','Seneri.webp','gxx-.png','Devilwalk.webp'];
    
            function changeImg(){
                document.getElementById('slideshow').src = imgArray[i];
    
                if(i< imgArray.length -1){
                    i++;
                }
                else{
                    i=0;
                }
                setTimeout("changeImg()", 3700);
            }
            document.body.addEventListener(onload, changeImg());
        </script>



<div id="kontenti1">
    <h2>Bad News Eagle Photo's </h2>
    <img id="slideshow1">

</div>
<footer>
    <div class="footer-container">
        <div class="footer-links">
            <h2>Sponsors</h2>
            <ul>
                <li><a href="#">Hellcase</a></li>
                <li><a href="">MadTech</a></li>
                <li><a href="#">Hummels</a></li>
                <li><a href="#">Gjirafa</a></li>
                <li><a href="#">Sky Broadband</a></li>
            </ul>
        </div>
        <div class="social-media">
            <h2>Social Media</h2>
            <ul>
                <li><a href="https://www.facebook.com/BadNewsEagles"><img class="img-socialmedia" src="https://www.unipile.com/wp-content/uploads/2022/06/logo-facebook-noir.png" alt=""></a></li>
                <li><a href="https://twitter.com/_badnewseagles"><img class="img-socialmedia" src="https://seeklogo.com/images/T/twitter-icon-circle-black-logo-35827D553B-seeklogo.com.png" alt=""></a></li>
               
            </ul>
        </div>
        <div class="contact-info">
            <h2>Contact Info</h2>
            <ul>
                <li>Email: info@badnews.com</li>
                <li>Phone: +383 43 730 558</li>
            </ul>
        </div>
    </div>
    <div class="footer-bottom">
        <p>&copy; 2022 Bad News Eagles. All rights reserved.</p>
    </div>
</footer>

<script>
    let j = 0;
    let imgArray1 = ['bnePlayer.webp','BneMajor.jpg','bnepinnacle-1130x635.jpg'];

    function changeImg1(){
        document.getElementById('slideshow1').src = imgArray1[j];

        if(j< imgArray1.length -1){
            j++;
        }
        else{
            j=0;
        }
       setTimeout("changeImg1()", 3000);
    }
    document.body.addEventListener(onload,changeImg1());
</script>
        
    </main>
    
</body>
</html>